...
class StashInterface:
    ...
    def __init__(self, conn):
        ...
        try:
            ...
            host = conn.get('Host') if '0.0.0.0' not in conn.get(
                'Host') or '' else 'localhost'
        except TypeError:
            # Pre stable 0.8
            host = 'localhost'
    ...
    def __callGraphQL(self, query, variables=None):
        ...
        if response.status_code == 200:
            ...
            sys.exit(
                "HTTP Error 401, Unauthorised. Cookie authentication most likely failed")
        else:
    ...
    def __findScenesByPathRegex(self, regex, page=1):
        ...
        result = self.__callGraphQL(query, variables)
        log.LogDebug(
            f"Regex found {result.get('findScenesByPathRegex').get('count')} scene(s) on page {page}")
